/*
 * Integration test: gauss_jordan_solve — correctness suite.
 *
 * Mirrors how embedded/flight software calls the solver:
 * stage inputs into fixed-size arrays, invoke, inspect Num_Status,
 * read x[] only on NUM_OK.
 */
...
static void load_A(double dst[NUM_MAX_N][NUM_MAX_N], const double* src, int n)
{
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j) dst[i][j] = src[i*n+j];
}
...
{
    run("2×2  3x+2y=12, x-y=1  →  x=14/5, y=9/5", [](std::string& e) {
        const double Af[]={3,2,1,-1}; const double bv[]={12,1};
        double A[NUM_MAX_N][NUM_MAX_N], b[NUM_MAX_N], x[NUM_MAX_N];
        load_A(A,Af,2); load_b(b,bv,2);
        auto s = gauss_jordan_solve(A,b,x,2);
        if (s!=NUM_OK) { e=num_status_str(s); return false; }
        if (!near(x[0],14.0/5)||!near(x[1],9.0/5)) { e="mismatch"; return false; }
        return true;
    });
}
...
{
    run("3×3  textbook  →  x=2, y=3, z=-1", [](std::string& e) {
        ...
        if (s!=NUM_OK) { e=num_status_str(s); return false; }
        if (!near(x[0],2)||!near(x[1],3)||!near(x[2],-1)) { e="mismatch"; return false; }
        return true;
    });
}

static void test_4x4_tridiagonal()
{
    run("4×4  tridiagonal  →  x={1,2,2,1}", [](std::string& e) {
        const double Af[]={4,-1,0,0,-1,4,-1,0,0,-1,4,-1,0,0,-1,4};
        ...
        for (int i=0;i<4;++i)
            if (!near(x[i],ex[i])) { e="mismatch"; return false; }
        return true;
    });
}
...
{
    run("4×4  identity  →  x=b", [](std::string& e) {
        double A[NUM_MAX_N][NUM_MAX_N]={}, b[NUM_MAX_N]={}, x[NUM_MAX_N]={};
        ...
        if (s!=NUM_OK) { e=num_status_str(s); return false; }
        for (int i=0;i<4;++i)
            if (!near(x[i],bv[i])) { e="mismatch"; return false; }
        return true;
    });
}
...
{
    run("2×2  x+y=1, x+y=2  →  NUM_ERR_NO_SOLUTION", [](std::string& e) {
...
{
    run("2×2  x+y=3, 2x+2y=6  →  NUM_ERR_INFINITE", [](std::string& e) {
...
{
    run("2×2  near-singular  →  no catastrophic result", [](std::string& e) {
        ...
        if (s==NUM_OK && (std::fabs(x[0])>1e8||std::fabs(x[1])>1e8))
            { e="NUM_OK with exploded solution"; return false; }
        return true;
    });
}
...
static void test_bad_input_zero()
{
    run("n=0  →  NUM_ERR_BAD_INPUT", [](std::string& e) {
        double A[NUM_MAX_N][NUM_MAX_N]={}, b[NUM_MAX_N]={}, x[NUM_MAX_N]={};
        if (gauss_jordan_solve(A,b,x,0)!=NUM_ERR_BAD_INPUT) { e="wrong status"; return false; }
        return true;
    });
}
...
static void test_bad_input_overflow()
{
    run("n>NUM_MAX_N  →  NUM_ERR_BAD_INPUT", [](std::string& e) {
        double A[NUM_MAX_N][NUM_MAX_N]={}, b[NUM_MAX_N]={}, x[NUM_MAX_N]={};
        if (gauss_jordan_solve(A,b,x,NUM_MAX_N+1)!=NUM_ERR_BAD_INPUT)
            { e="wrong status"; return false; }
        return true;
    });
}
...
static void test_inputs_not_modified()
{
    run("A and b are not modified by the solver", [](std::string& e) {
        ...
        if (std::memcmp(A,Ac,sizeof(A))!=0) { e="A modified"; return false; }
        if (std::memcmp(b,bc,sizeof(b))!=0) { e="b modified"; return false; }
        return true;
    });
}
...
int main()
{
    std::cout << "gauss_jordan_solve — integration tests\n";
    std::cout << "=======================================\n\n";
    test_1x1();
    test_2x2_unique();
    test_3x3_unique();
